...
if cuda:
    ...
    ggenerator.to(device) # cuda()
    gdiscriminator.to(device) # cuda()
...
feature_extractor = FE_MLP().to(device) # .cuda()
...
def sample_image(n_row, batches_done):
    ...
    save_image(gen_imgs.data, "imgFedCGAN/F" + str(opt.num_users) + "_%d.png" % batches_done, nrow=n_row, normalize=True)
...
for epoch in range(1,1+opt.n_epochs):
    for di in range(opt.num_users):
        for i, (imgs, labels) in enumerate(dataloaders[di]):
            ...
            # Configure input
            imgs = imgs.to(device)
            imgs = feature_extractor(imgs) # x.view(-1, self.feature_size*self.feature_size) in CVAE.forward             
...
torch.save(gw, 'models/save/F' + str(opt.num_users)+ '_'+ str(opt.n_epochs)+ 'gan_generator.pt')
torch.save(dw, 'models/save/F' + str(opt.num_users)+ '_'+ str(opt.n_epochs)+ 'gan_discriminator.pt')