...
class Model(nn.Module):
    ...
    def encoder(self,x,y):
        y = torch.argmax(y, dim=1).reshape((y.shape[0],1,1,1)) # (batch_size, 1, 1, 1)
        y = torch.ones(x.shape).to(device)*y # x.shape (batch_size, 1, 28, 28)
        t = torch.cat((x,y),dim=1) # (batch_size, 2, 28, 28)

        t = F.relu(self.conv1(t))
        t = F.relu(self.conv2(t))
        t = t.reshape((x.shape[0], -1)) # -1, 512 (32*4*4)
...
def test(epoch, model, test_loader):
    ...
    with torch.no_grad():
        for i,(x,y) in enumerate(test_loader):
            try: # x (batch_size, 1, 28, 28) y (batch_size)
                label = np.zeros((x.shape[0], 10)) # (batch size, 10)
                label[np.arange(x.shape[0]), y] = 1 # (batch size, 10)
                label = torch.tensor(label)
